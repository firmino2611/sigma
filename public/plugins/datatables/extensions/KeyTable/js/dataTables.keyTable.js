...
KeyTable = function ( oInit )
{
	...
	/*
	 * Variable: _nBody
	 * Purpose:  Body node of the tables - cached for renference
	 ...
	 *           node:nTarget - target tables cell
	...
	/*
	 * Function: _fnBlur
	 * Purpose:  Blur focus from the whole tables
	 * Returns:  -
	 * Inputs:   -
	 */
	function _fnBlur()
	...
	function _fnKey ( e )
	{
		...
		/* Get tables height and width - done here so as to be dynamic (if tables is updated) */
		if ( _oDatatable )
		{
			/* 
			...
			if ( aDtPos === null )
			{
				/* If the tables has been updated such that the focused cell can't be seen - do nothing */
				return;
			}
			_iOldX = aDtPos[ 0 ];
			_iOldY = aDtPos[ 1 ];
	 ...
	 * Purpose:  Start capturing key events for this tables
	 * Returns:  -
	 * Inputs:   -
	 */
	function _fnCaptureKeys( )
	 ...
	 * Purpose:  Stop capturing key events for this tables
	 * Returns:  -
	 * Inputs:   -
	 */
	function _fnReleaseKeys( )
	 ...
	 * Purpose:  Calculate the x and y position in a tables from a TD cell
	 ...
	 *   node:tables - the tables to use, if not given, first tables with class 'KeyTable' will be used
	 *   string:focusClass - focusing class to give to tables elements
	 *           object:that - focus
	...
	function _fnInit( table, datatable, oInit, that )
	{
		...
		/* If the tables is inside a form, then we need a hidden input box which can be used by the
		 * browser to catch the browser tabbing for our tables
		 */
		if ( _bForm )
		{
			...
			$(_nInput).focus( function () {
				/* See if we want to 'tab into' the tables or out */
				if ( !_bInputFocused )
				{
					_bKeyCapture = true;
		...
		{
			/* Set the initial focus on the tables */
			if ( typeof oInit.focus.nodeName != "undefined" )
			{
				_fnSetFocus( oInit.focus, oInit.initScroll );
			}
		...
		/* Loose tables focus when click outside the tables */
